...
import shaper.mapping.metadata.shex.NodeConstraint;
import shaper.mapping.metadata.shex.ShExSchemaFactory;
import shaper.mapping.metadata.shex.Shape;
...
public class R2RMLBasedRDFMapper extends RDFMapper {
    //-> for R2RML parser
    ...
    public R2RMLBasedRDFMapper(String propertiesFile) {
    ...
    public File generateRDFFile() {
        if (!loadPropertiesFile()) return null;
            ...
            switch (extension) {
                ...
                case ShEx:
                    ...
                    if (defaultNamespace != null)
                        writer.println(BASE + SPACE + LT + defaultNamespace + GT); // for a default RDF Graph namespace by r2rml
                    ...
                    for (Map.Entry<String, String> entry: entrySet)
                        writer.println(PREFIX + SPACE + entry.getKey() + COLON + SPACE + LT + entry.getValue() + GT);
                    writer.println();
                    break;
            }
        } catch (Exception e) {
                ...
                if (objectMap.isPresent()) {
                    if (NodeConstraint.isPossibleToHaveXSFacet(objectMap.get())) {
                        ...
                        if (nodeConstraintID != null && nodeConstraint != null) {
                            String id = shExSchema.getPrefix() + COLON + nodeConstraintID;
                            writer.println(id + SPACE + nodeConstraint);
                        }
                    }
                }
            }
    ...
    public File generateShExFile() {
        ...
        shExSchema = ShExSchemaFactory.buildShExSchemaModel(r2rmlModel);

        preProcess(Extension.ShEx);
        writeDirectives(DirectMappingBasedRDFMapper.Extension.ShEx);
        writeShEx();
        postProcess();
